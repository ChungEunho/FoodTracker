...
def _to_data_url(path: Path) -> str:
    """이미지를 표준 JPEG data URL로 변환한다.

    PIL이 있으면 파일을 열어 표준 JPEG로 재인코딩하므로, HEIC·비표준 JPEG 등
    일부 공급자가 인식하지 못하는 포맷 문제를 방지한다.
    PIL이 없으면 파일 원본을 그대로 base64로 인코딩한다.
    """
    if _PIL:
...
def recognize(image_path: str) -> dict:
    ...
    if size > _MAX_IMAGE_BYTES:
        print(f"경고: 이미지 크기가 큽니다 ({size / 1024 / 1024:.1f}MB > 10MB). 전송이 느려질 수 있습니다.")
