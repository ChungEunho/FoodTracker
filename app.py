...
class App(tk.Tk):
    ...
    def _apply_theme(self):
        ...
        style.configure("TLabelframe", background=C["bg"], bordercolor=C["border"],
                        font=("Helvetica Neue", 11, "bold"), foreground=C["text"])
        ...
        style.map("TNotebook.Tab",
                  background=[("selected", C["surface"]), ("active", "#D9E4F5")],
        ...
        style.configure("TButton",
                        ...
                        foreground=C["text"])
        style.map("TButton",
                  background=[("active", "#C8D0DC"), ("disabled", "#E5E7EB")],
        ...
        style.map("Primary.TButton",
                  background=[("active", C["primary_dk"]), ("disabled", "#93B8E0")],
        ...
        style.map("Danger.TButton",
                  background=[("active", "#FECACA"), ("disabled", "#F5F5F5")],
        ...
        style.map("Treeview",
                  ...
                  foreground=[("selected", C["primary_dk"])])
    ...
    def _build_log(self, nb):
        ...
        ttk.Combobox(
            opt_f, textvariable=self._meal_var, values=list(MEAL_TYPES),
            state="readonly", width=8,
        ).grid(row=0, column=1, padx=(4, 24), sticky="w")

        ttk.Label(opt_f, text="날짜 (YYYY-MM-DD):").grid(row=0, column=2, sticky="w")
        self._log_date = tk.StringVar(value=date.today().isoformat())
        ttk.Entry(opt_f, textvariable=self._log_date, width=13).grid(
            row=0, column=3, padx=(4, 4), sticky="w"
        )
        ttk.Button(
            opt_f, text="오늘", width=4, style="Small.TButton",
            command=lambda: self._log_date.set(date.today().isoformat()),
        ).grid(row=0, column=4, padx=(0, 2), sticky="w")
        ttk.Button(
            opt_f, text="어제", width=4, style="Small.TButton",
            command=lambda: self._log_date.set((date.today() - timedelta(days=1)).isoformat()),
        ...
        ttk.Entry(opt_f, textvariable=self._log_time, width=7).grid(
        ...
        self._log_status_lbl = ttk.Label(
            ctrl,
            textvariable=self._log_status,
            foreground=C["text_sub"],
            font=("Helvetica Neue", 11),
        )
        ...
        self._log_tree.tag_configure(
            "total",
            font=("Helvetica Neue", 11, "bold"),
            background=C["total_bg"],
        )

        # ── 저장/초기화 버튼 (검색·직접입력 모드용) ─────────────────────────
        self._log_save_bar = ttk.Frame(f)
        self._log_save_bar.pack(fill="x", padx=12, pady=(0, 6))

        self._log_save_btn = ttk.Button(
            self._log_save_bar, text="저장", style="Primary.TButton",
            command=self._do_save_result, state="disabled"
        )
        self._log_save_btn.pack(side="left")
        ttk.Button(
            self._log_save_bar, text="초기화", style="Small.TButton",
            command=self._clear_log_tree,
    ...
    def _build_log_image_tab(self, f):
        ...
        ttk.Entry(row, textvariable=self._img_var, state="readonly").pack(
            side="left", fill="x", expand=True
        )
        ttk.Button(row, text="찾아보기…", command=self._pick_image).pack(
            side="left", padx=(6, 0)
        )
        ...
        self._log_btn = ttk.Button(
            f, text="분석 & 저장", style="Primary.TButton", command=self._do_analyze
        )
    ...
    def _build_log_search_tab(self, f):
        ...
        ttk.Entry(row, textvariable=self._srch_brand, width=16).pack(
            side="left", padx=(4, 16)
        )

        ttk.Label(row, text="메뉴명:").pack(side="left")
        self._srch_menu = tk.StringVar()
        ttk.Entry(row, textvariable=self._srch_menu, width=22).pack(
            side="left", padx=(4, 12)
        )

        self._srch_btn = ttk.Button(
            row, text="검색", style="Primary.TButton", command=self._do_search
        )
        self._srch_btn.pack(side="left")

        ttk.Label(
            f,
            text="인터넷에서 칼로리 및 영양 정보를 검색합니다. 검색 후 결과를 확인하고 '저장'을 누르세요.",
            foreground=C["text_sub"],
            font=("Helvetica Neue", 10),
            background=C["bg"],
    ...
    def _build_log_manual_tab(self, f):
        ...
        for col, (lbl, w) in enumerate(zip(labels, widths)):
            ttk.Label(fields_f, text=lbl, font=("Helvetica Neue", 10)).grid(
                row=0, column=col, padx=(0 if col == 0 else 4, 0), sticky="w"
            )
            var = tk.StringVar()
            self._manual_vars.append(var)
            ttk.Entry(fields_f, textvariable=var, width=w).grid(
                row=1, column=col, padx=(0 if col == 0 else 4, 0), sticky="ew"
            )

        btn_row = ttk.Frame(f)
        btn_row.pack(fill="x", pady=(6, 0))
        ttk.Button(
            btn_row, text="항목 추가", style="Primary.TButton",
            command=self._do_add_manual,
        ).pack(side="left")
        ttk.Label(
            btn_row,
            text="항목을 하나씩 추가한 뒤 '저장'을 누르세요.",
            foreground=C["text_sub"],
            font=("Helvetica Neue", 10),
            background=C["bg"],
    ...
    def _do_analyze(self):
        ...
        def worker():
            try:
                ...
                self._post(lambda: (
                    self._log_status_fg(C["text_sub"]),
                    self._log_status.set("📊 [2/2] 영양 정보를 계산하는 중…"),
                ))
    ...
    def _analyze_done(self, result, path, meal, meal_date, meal_time):
        ...
        for idx, it in enumerate(items):
            ...
            self._log_tree.insert("", "end", tags=(tag,), values=(
                it.get("name", ""),
                it.get("weight_g", 0), it.get("calories_kcal", 0),
                it.get("carbs_g", 0), it.get("protein_g", 0),
                it.get("fat_g", 0), it.get("sugar_g", 0),
            ))
        if total:
            self._log_tree.insert("", "end", tags=("total",), values=(
                "── 합계 ──",
                total.get("weight_g", 0), total.get("calories_kcal", 0),
                total.get("carbs_g", 0), total.get("protein_g", 0),
                total.get("fat_g", 0), total.get("sugar_g", 0),
            ))
        ...
        try:
            with closing(get_conn()) as conn, conn:
                conn.execute(
                    "INSERT INTO meals "
                    "(date, meal_type, image_path, items_json, total_json, created_at, meal_time) "
                    "VALUES (?,?,?,?,?,?,?)",
                    (
                        meal_date, meal, path,
    ...
    def _do_search(self):
        ...
        def worker():
            try:
                ...
                self._post(
                    lambda r=result, fn=found_name, ie=is_exact:
                    self._search_done(r, fn, ie, brand, menu)
                )
            except Exception as e:
                self._post(lambda err=str(e): self._log_error(err))
    ...
    def _search_done(self, result, found_name, is_exact, brand, menu):
        ...
        if not is_exact:
            ok = messagebox.askyesno(
                "검색 결과 확인",
                f"'{brand} {menu}'를 정확히 찾지 못했습니다.\n\n"
                f"대신 아래 제품의 영양 정보를 사용하시겠습니까?\n\n"
                f"  ▶  {found_name}",
            )
            if not ok:
                self._log_status_fg(C["text_sub"])
    ...
    def _do_add_manual(self):
        ...
        self._pending_total = {
            k: sum(it.get(k, 0) for it in self._pending_items) for k in keys_num
        }
        self._pending_image_path = ""
        self._update_log_tree()

        # 입력 필드 초기화 (음식명 빼고 숫자 필드만)
    ...
    def _update_log_tree(self):
        ...
        if t:
            self._log_tree.insert("", "end", tags=("total",), values=(
                "── 합계 ──",
                t.get("weight_g", 0), t.get("calories_kcal", 0),
                t.get("carbs_g", 0), t.get("protein_g", 0),
                t.get("fat_g", 0), t.get("sugar_g", 0),
            ))
        ...
        try:
            with closing(get_conn()) as conn, conn:
                conn.execute(
                    ...
                    "VALUES (?,?,?,?,?,?,?)",
                    (
                        meal_date, meal, self._pending_image_path,
                        json.dumps(self._pending_items, ensure_ascii=False),
                        json.dumps(self._pending_total, ensure_ascii=False),
                        datetime.now().isoformat(),
                        meal_time,
                    ),
                )
    ...
    def _build_show(self, nb):
        ...
        ttk.Entry(ctrl, textvariable=self._show_date, width=13).pack(
            side="left", padx=(4, 4)
        )
        ttk.Button(
            ctrl, text="오늘", width=4, style="Small.TButton",
            command=lambda: self._show_date.set(date.today().isoformat()),
        ).pack(side="left", padx=(0, 2))
        ttk.Button(
            ctrl, text="어제", width=4, style="Small.TButton",
            command=lambda: self._show_date.set((date.today() - timedelta(days=1)).isoformat()),
        ).pack(side="left", padx=(0, 10))
        ttk.Button(
            ctrl, text="조회", style="Primary.TButton", command=self._do_show
        ).pack(side="left", padx=(0, 10))

        self._show_del_btn = ttk.Button(
            ctrl, text="선택 식사 삭제", style="Danger.TButton",
            command=self._do_delete_show_meal, state="disabled"
        )
        self._show_del_btn.pack(side="left")

        self._show_summary = tk.StringVar()
        ttk.Label(
            f, textvariable=self._show_summary,
    ...
    def _do_show(self):
        ...
        with closing(get_conn()) as conn:
            rows = conn.execute(
                "SELECT * FROM meals WHERE date = ? ORDER BY meal_type, created_at",
                (self._show_date.get(),),
            ).fetchall()

        if not rows:
            self._show_tree.insert(
                "", "end", tags=("empty",),
                values=("기록이 없습니다. '식사 기록' 탭에서 추가하세요.", "", "", "", "", "", ""),
            )
            return
        ...
        for mt in MEAL_TYPES:
            ...
            for row in meal_rows:
                ...
                iid_time = self._show_tree.insert(
                    "", "end", tags=("time_row",),
                    values=(f"   · {t}", "", "", "", "", "", ""),
                )
                ...
                for it in items:
                    ...
                    data_row_idx += 1
                    iid = self._show_tree.insert("", "end", tags=(tag,), values=(
                        f"     {it.get('name', '')}",
                        it.get("weight_g", 0), it.get("calories_kcal", 0),
                        it.get("carbs_g", 0), it.get("protein_g", 0),
                        it.get("fat_g", 0), it.get("sugar_g", 0),
                ...
                iid_sub = self._show_tree.insert("", "end", tags=("subtotal",), values=(
                    "   소계",
                    total.get("weight_g", 0), total.get("calories_kcal", 0),
                    total.get("carbs_g", 0), total.get("protein_g", 0),
                    total.get("fat_g", 0), total.get("sugar_g", 0),
        ...
        self._show_summary.set(
            f"하루 합계:  {day['calories_kcal']} kcal  |  "
            f"탄수 {day['carbs_g']}g   단백 {day['protein_g']}g   "
            f"지방 {day['fat_g']}g   당류 {day['sugar_g']}g"
        )
    ...
    def _do_delete_show_meal(self):
        sel = self._show_tree.selection()
        meal_ids = {
            self._show_meal_map[iid]
            for iid in sel
            if iid in self._show_meal_map
        }
        if not meal_ids:
            return
        ...
        self._do_show()           # 일별 조회 새로 고침
        self._load_records()      # 기록 관리 새로 고침
    ...
    def _build_summary(self, nb):
        ...
        ttk.Entry(ctrl, textvariable=self._from_var, width=13).pack(
            side="left", padx=(4, 4)
        )
        ttk.Button(
            ctrl, text="어제", width=4, style="Small.TButton",
            command=lambda: self._from_var.set((date.today() - timedelta(days=1)).isoformat()),
        ).pack(side="left", padx=(0, 12))

        ttk.Label(ctrl, text="종료일:").pack(side="left")
        self._to_var = tk.StringVar(value=date.today().isoformat())
        ttk.Entry(ctrl, textvariable=self._to_var, width=13).pack(
            side="left", padx=(4, 4)
        )
        ttk.Button(
            ctrl, text="오늘", width=4, style="Small.TButton",
            command=lambda: self._to_var.set(date.today().isoformat()),
        ).pack(side="left", padx=(0, 12))

        ttk.Button(
            ctrl, text="요약", style="Primary.TButton", command=self._do_summary
        ).pack(side="left")
    ...
    def _do_summary(self):
        ...
        with closing(get_conn()) as conn:
            rows = conn.execute(
                "SELECT date, total_json FROM meals "
                "WHERE date BETWEEN ? AND ? ORDER BY date",
                (self._from_var.get(), self._to_var.get()),
            ).fetchall()
        ...
        for idx, (d, t) in enumerate(sorted(day_map.items())):
            tag = "even" if idx % 2 == 0 else "odd"
            self._sum_tree.insert("", "end", tags=(tag,), values=(
                d, t["calories_kcal"], t["carbs_g"], t["protein_g"], t["fat_g"]
            ))
            for k in totals:
                totals[k] += t[k]

        n = len(day_map)
        self._sum_tree.insert("", "end", tags=("avg",), values=(
            "평균",
            round(totals["calories_kcal"] / n, 1),
            round(totals["carbs_g"] / n, 1),
            round(totals["protein_g"] / n, 1),
            round(totals["fat_g"] / n, 1),
        ))
    ...
    def _build_records(self, nb):
        ...
        self._del_btn = ttk.Button(
            ctrl, text="선택 삭제", style="Danger.TButton",
    ...
    def _load_records(self):
        ...
        with closing(get_conn()) as conn:
            rows = conn.execute(
                "SELECT id, date, meal_type, meal_time, total_json, created_at "
                "FROM meals ORDER BY date DESC, meal_time, created_at DESC"
            ).fetchall()
        ...
        for idx, row in enumerate(rows):
            ...
            self._rec_tree.insert("", "end", tags=(tag,), values=(
                row["id"], row["date"], row["meal_type"],
                row["meal_time"] or "",
                total.get("calories_kcal", 0),
                row["created_at"][:19],
            ))
